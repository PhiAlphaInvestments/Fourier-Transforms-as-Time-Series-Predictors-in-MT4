...
input Power InpDepthOfReturns = Mid;   // Fast EMA Period
input int MA = 20;
...
double FFT_of_Returns(const double &close[],int pos){
   ...
   pred = FFtransform.FFT_Calculate_Prediction( InputVals,5);
    ...
    for( int i = 0; i < 5; i++){
       ...
       mu += pred[InpDepthOfReturns+i].real ;
    ...
    double FinalPred = mu/double(MA);  // pred[InpDepthOfReturns].real ;  //(pred[InpDepthOfReturns+2].real - pred[InpDepthOfReturns+1].real ]    ) ;
    //Print(FinalPred);
    return  FinalPred  ;
